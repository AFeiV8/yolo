...
def non_max_suppression(
    ...
    xc = prediction[..., 4] > conf_thres  # 用candidates值过滤一部分，shape(1,n)
    ...
    for xi, x in enumerate(prediction):  # index； image inference，shape(0,nc+5)
        # Apply constraints
        # x[((x[..., 2:4] < min_wh) | (x[..., 2:4] > max_wh)).any(1), 4] = 0  # width-height
        torch.cuda.empty_cache()
        x = x[xc[xi]]  # 用bool值列表取出被confidence阈值过滤剩下的预测信息
        ...
        # Box/Mask
        box = xywh2xyxy(x[:, :4])  # 转换为左上角点右下角点的坐标的形式 to (x1, y1, x2, y2)
